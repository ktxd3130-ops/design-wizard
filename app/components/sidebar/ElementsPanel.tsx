...
export function ElementsPanel({ fabricRef, brandConfig }: ElementsPanelProps) {
    return (
        <div className="flex-1 overflow-y-auto p-6 space-y-10">
            <div className="relative sticky top-0 z-10 bg-[var(--surface-2)] pb-4">
                <Search size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 mt-[-4px] text-[var(--ui-30)]" />
                <input placeholder="Search elements" className="w-full bg-[var(--ui-10)] border border-[var(--ui-10)] rounded-xl pl-11 pr-4 py-3 text-sm text-[var(--ui-80)] placeholder-white/30 focus:outline-none focus:border-violet-500/50 transition-all" />
            </div>
