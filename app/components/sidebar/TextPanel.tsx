    ...
    return (
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
            <div className="relative">
                <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--ui-30)]" />
                <input placeholder="Search fonts and combinations" className="w-full bg-[var(--ui-10)] border border-[var(--ui-10)] rounded-lg pl-10 pr-3 py-2.5 text-sm text-[var(--ui-80)] placeholder-white/30 focus:outline-none focus:border-violet-500/50 focus:ring-1 focus:ring-violet-500/30 transition-all" />
            </div>

            <button draggable onDragStart={(e) => { e.dataTransfer.setData('application/design-wizard-type', 'text'); e.dataTransfer.setData('application/design-wizard-data', 'Your text here'); e.dataTransfer.effectAllowed = 'copy'; }} onClick={() => handleAddText()} className="w-full flex items-center justify-center gap-2 bg-violet-600 hover:bg-violet-500 text-[var(--ui-100)] py-2.5 rounded-lg font-semibold text-sm transition-colors shadow-lg shadow-violet-500/20 cursor-grab active:cursor-grabbing">
                <Type size={18} /> Add a text box
            </button>

            <button className="w-full flex items-center justify-center gap-2 bg-[var(--ui-5)] hover:bg-[var(--ui-10)] border border-[var(--ui-10)] text-[var(--ui-100)] py-2.5 rounded-lg font-medium text-sm transition-colors cursor-pointer">
                <Wand2 size={16} className="text-violet-400" /> Magic Write
            </button>

            <p className="text-[11px] text-[var(--ui-30)] uppercase tracking-wider font-semibold mt-6 mb-2">Default text styles</p>
            ...
            <p className="text-[11px] text-[var(--ui-30)] uppercase tracking-wider font-semibold mt-6 mb-2">Dynamic text</p>
            <button className="w-full flex items-center gap-3 px-3 py-2 bg-[var(--ui-5)] hover:bg-[var(--ui-10)] rounded-lg border border-[var(--ui-5)] transition-colors cursor-pointer group">
                <div className="w-10 h-10 bg-gradient-to-br from-orange-600 to-amber-500 rounded flex items-center justify-center text-[var(--ui-90)] text-xs font-bold ring-1 ring-white/10">
                    [1]
                </div>
            ...
            <div className="grid grid-cols-3 gap-2 pb-4">
                {[1, 2, 3].map(i => (
                    <div key={i} className="aspect-square bg-gradient-to-br from-violet-500/20 to-fuchsia-500/20 rounded-lg border border-[var(--ui-10)] flex items-center justify-center cursor-pointer hover:border-[var(--ui-30)] transition-colors">
                        <Sparkles size={16} className="text-[var(--ui-40)]" />
                    </div>
                ))}
            </div>

            <p className="text-[11px] text-[var(--ui-30)] uppercase tracking-wider font-semibold mt-2 mb-2 border-t border-[var(--ui-5)] pt-4">Font combinations</p>
            {[
